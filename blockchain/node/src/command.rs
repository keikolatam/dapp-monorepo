...
//! Keiko command implementations.

use crate::{
    chain_spec,
    cli::{Cli, RelayChainCli, Subcommand},
    service::{new_partial, KeikoRuntimeExecutor},
};
...
impl SubstrateCli for RelayChainCli {
    ...
    fn description() -> String {
		...
		passed to the parachain node, while the arguments provided after -- will be passed \
		to the relay chain node.\n\n\
		keiko-node <parachain-args> -- <relay-chain-args>"
            .into()
    }
...
pub fn run() -> Result<()> {
    ...
    match &cli.subcommand {
        ...
        Some(Subcommand::Benchmark(cmd)) => {
            let runner = cli.create_runner(cmd)?;
            // Switch on the specific benchmark sub-command-
            match cmd {
                BenchmarkCmd::Pallet(cmd) => {
                    if cfg!(feature = "runtime-benchmarks") {
                        runner.sync_run(|config| {
                            cmd.run::<sp_runtime::traits::HashFor<Block>, KeikoRuntimeExecutor>(
                                config,
                            )
                        })
                    } else {
            ...
            runner.async_run(|config| {
                ...
                Ok((
                    cmd.run::<Block, ExtendedHostFunctions<
                        ...
                        <KeikoRuntimeExecutor as NativeExecutionDispatch>::ExtendHostFunctions,
...
#[cfg(feature = "try-runtime")]
fn timestamp_with_aura_info(slot_duration: u64) -> impl sp_inherents::InherentDataProvider {
