...
fn comment_too_long_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            ReputationSystem::create_rating(RuntimeOrigin::signed(1), 2, 5, long_comment, None,),
...
fn bidirectional_rating_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(
            Event::BidirectionalRatingCompleted {
                interaction_id: 1,
                student_rating: 1,
                tutor_rating: 2,
            }
...
fn get_ratings_by_type_works() {
    new_test_ext().execute_with(|| {
        ...
        // Get ratings by type
        let student_ratings =
            ReputationSystem::get_ratings_by_type(&2, &crate::RatingType::StudentToTutor);
        assert_eq!(student_ratings.len(), 1);
        assert_eq!(student_ratings[0], 1);

        let peer_ratings =
            ReputationSystem::get_ratings_by_type(&2, &crate::RatingType::PeerToPeer);
        assert_eq!(peer_ratings.len(), 1);
        assert_eq!(peer_ratings[0], 2);

        let general_ratings =
            ReputationSystem::get_ratings_by_type(&2, &crate::RatingType::General);
