...
use crate::{Error, Event, LearningPreferences, LearningProfile, LearningStyle, PrivacySettings};
    ...
    mod with_mock {
        ...
        fn create_passport_works() {
            new_test_ext().execute_with(|| {
                ...
                assert_ok!(LifeLearningPassport::create_passport(
        ...
        fn passport_verification_works() {
            new_test_ext().execute_with(|| {
                ...
                assert!(LifeLearningPassport::verify_passport_authenticity(
                    &1, &passport
                ));
...
fn helper_functions_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(LifeLearningPassport::get_interaction_count(&1), 0);
        assert_eq!(
            LifeLearningPassport::get_passport_creation_time(&1),
            Some(1)
        );
...
fn revoke_shareable_link_works() {
    new_test_ext().execute_with(|| {
        ...
        if let RuntimeEvent::LifeLearningPassport(Event::ShareableLinkCreated { link_id, .. }) =
            &link_created_event.event
        {
            // Revoke the link
...
fn record_adaptive_action_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            profile.adaptive_history[0].trigger,
            b"low_engagement".to_vec()
        );
        assert_eq!(
            profile.adaptive_history[0].action,
            b"content_adjustment".to_vec()
        );
...
fn update_profile_from_interaction_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            profile.areas_for_improvement[0].domain,
            b"mathematics".to_vec()
        );
