...
impl cumulus_pallet_xcmp_queue::Config for Runtime {
    ...
    type PriceForSiblingDelivery =
        polkadot_runtime_common::xcm_sender::NoPriceForSiblingDelivery<ParaId>;
}
...
mod xcm_config {
    ...
    use staging_xcm_builder::{
        ...
        NativeAsset, ParentAsSuperuser, ParentIsPreset, RelayChainAsNative,
    ...
    pub struct DenyThenTry<Deny, Allow>(
        sp_std::marker::PhantomData<Deny>,
        sp_std::marker::PhantomData<Allow>,
    )
    where
        Deny: ShouldExecute,
        Allow: ShouldExecute;
...
impl BuildStorage for RuntimeGenesisConfig {
    fn build_storage(&self) -> Result<sp_runtime::Storage, String> {
        ...
        // Initialize session
        pallet_session::GenesisConfig::<Runtime> { keys: vec![] }
            .assimilate_storage(&mut storage)?;
