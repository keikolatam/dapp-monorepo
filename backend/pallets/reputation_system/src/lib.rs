...
pub mod pallet {
	...
	pub type CommentResponses<T: Config> = StorageMap<
		...
		Blake2_128Concat,
		RatingId,
		BoundedVec<CommentResponse<T::AccountId, BlockNumberFor<T>>, ConstU32<MAX_COMMENT_RESPONSES>>,
		ValueQuery,
	>;
	...
	impl<T: Config> Pallet<T> {
			...
			match rating_type {
				...
				RatingType::TutorToStudent | RatingType::StudentToTutor => {
					if let Some(interaction_id) = interaction_id {
						Self::handle_bidirectional_rating(interaction_id, actual_rating_id, &rating_type)?;
					}
				},
				_ => {},
			}
		...
		pub fn add_comment_response(
			...
			let bounded_content: BoundedVec<u8, ConstU32<MAX_COMMENT_LENGTH>> = 
				response_content.try_into().map_err(|_| Error::<T>::CommentTooLong)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn handle_bidirectional_rating(
			...
			match rating_type {
				RatingType::StudentToTutor => {
					ensure!(rating_pair.0.is_none(), Error::<T>::BidirectionalRatingExists);
					rating_pair.0 = Some(rating_id);
				},
				RatingType::TutorToStudent => {
					ensure!(rating_pair.1.is_none(), Error::<T>::BidirectionalRatingExists);
		...
		pub fn get_rating_pair(interaction_id: &InteractionId) -> (Option<RatingId>, Option<RatingId>) {
			Self::rating_pairs(interaction_id)
		}

		/// Get all responses for a rating comment
		pub fn get_comment_responses(rating_id: &RatingId) -> Vec<CommentResponse<T::AccountId, BlockNumberFor<T>>> {
		...
		pub fn calculate_weighted_reputation(user: &T::AccountId) -> ReputationScore<BlockNumberFor<T>> {
