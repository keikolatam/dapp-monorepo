...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_passport(origin: OriginFor<T>) -> DispatchResult {
            ...
            // Ensure passport doesn't already exist
                ...
                !Passports::<T>::contains_key(&who),
                Error::<T>::PassportAlreadyExists
            );
        ...
        pub fn add_interaction(
            ...
            Self::deposit_event(Event::InteractionAdded {
                owner: who,
                interaction_id,
            });
        ...
        pub fn update_learning_profile(
            ...
            // Check skills limits
                ...
                learning_profile.areas_for_improvement.len()
                    <= T::MaxSkillsPerProfile::get() as usize,
                Error::<T>::TooManySkills
            );
        ...
        #[pallet::call_index(5)]
        #[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
        pub fn revoke_shareable_link(origin: OriginFor<T>, link_id: Vec<u8>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            Self::deposit_event(Event::ShareableLinkRevoked {
                owner: who,
                link_id,
            });
        ...
        pub fn update_skill(
            ...
            // Validate skill parameters
                ...
                domain.len() <= T::MaxTextLength::get() as usize,
                Error::<T>::TextTooLong
            );
        ...
        #[pallet::call_index(7)]
        #[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
        pub fn toggle_auto_update(origin: OriginFor<T>, enabled: bool) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn record_adaptive_action(
            ...
            // Validate parameters
                ...
                trigger.len() <= T::MaxTextLength::get() as usize,
                Error::<T>::TextTooLong
            );
            ensure!(
                action.len() <= T::MaxTextLength::get() as usize,
                Error::<T>::TextTooLong
            );
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn update_profile_from_interaction(
            ...
            Passports::<T>::try_mutate(owner, |passport_opt| -> DispatchResult {
                ...
                if let Some(ref mut profile) = passport.learning_profile {
                    ...
                    if !skill_updated
                        && profile.areas_for_improvement.len()
                            < T::MaxSkillsPerProfile::get() as usize
                    {
        ...
        pub fn get_learning_recommendations(owner: &T::AccountId) -> Vec<Vec<u8>> {
            if let Some(passport) = Self::passports(owner) {
        ...
        pub fn get_adaptive_insights(owner: &T::AccountId) -> Vec<AdaptiveAction> {
            if let Some(passport) = Self::passports(owner) {
                if let Some(profile) = passport.learning_profile {
        ...
        pub fn calculate_learning_progress(owner: &T::AccountId) -> u8 {
            if let Some(passport) = Self::passports(owner) {
                if let Some(profile) = passport.learning_profile {
                    let total_skills =
                        profile.strengths.len() + profile.areas_for_improvement.len();
                    if total_skills == 0 {
                        return 0;
                    }

                    let total_level: u32 = profile
                        .strengths
                        .iter()
                        .map(|s| s.level as u32)
                        .sum::<u32>()
                        + profile
