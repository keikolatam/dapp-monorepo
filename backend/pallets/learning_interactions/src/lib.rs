...
pub mod pallet {
    ...
    pub type Courses<T: Config> =
        StorageMap<_, Blake2_128Concat, CourseId, Course<T::AccountId>, OptionQuery>;

    /// Storage for classes
    #[pallet::storage]
    #[pallet::getter(fn classes)]
    pub type Classes<T: Config> =
        StorageMap<_, Blake2_128Concat, ClassId, Class<T::AccountId>, OptionQuery>;

    /// Storage for tutorial sessions
    #[pallet::storage]
    #[pallet::getter(fn tutorial_sessions)]
    pub type TutorialSessions<T: Config> =
        StorageMap<_, Blake2_128Concat, TutorialId, TutorialSession<T::AccountId>, OptionQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        TutorialSessionEnded { tutorial_id: TutorialId },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_interaction(
            ...
            if let Some(tid) = tutorial_id {
                let mut tutorial =
                    Self::tutorial_sessions(&tid).ok_or(Error::<T>::TutorialSessionNotFound)?;
        ...
        pub fn end_tutorial_session(
            ...
            let mut tutorial =
                Self::tutorial_sessions(&tutorial_id).ok_or(Error::<T>::TutorialSessionNotFound)?;
    ...
    impl<T: Config> Pallet<T> {
                ...
                match item.evidence_type {
                    ...
                    EvidenceType::Url => {
                        ...
                        let content_str = sp_std::str::from_utf8(&item.content)
                            ...
                            content_str.starts_with("http://")
                                || content_str.starts_with("https://"),
                            Error::<T>::InvalidXAPIStructure
                        );
                    }
        ...
        pub fn get_interactions_by_tutorial(
            tutorial_id: TutorialId,
        ) -> Vec<LearningInteraction<T::AccountId>> {
        ...
        pub fn get_interactions_by_class(
            class_id: ClassId,
        ) -> Vec<LearningInteraction<T::AccountId>> {
        ...
        pub fn create_verb(verb_type: &str) -> Verb {
            ...
            match verb_type {
                "experienced" => {
                    ...
                    display.insert(b"es".to_vec(), "experimentó".as_bytes().to_vec());
                    Verb {
                ...
                "completed" => {
                    ...
                    display.insert(b"es".to_vec(), "completó".as_bytes().to_vec());
                    Verb {
                ...
                "answered" => {
                    ...
                    display.insert(b"es".to_vec(), "respondió".as_bytes().to_vec());
                    Verb {
                ...
                "asked" => {
                    ...
                    display.insert(b"es".to_vec(), "preguntó".as_bytes().to_vec());
                    Verb {
                ...
                _ => {
                    ...
                    Verb {
                        id: format!("http://keiko-dapp.xyz/verbs/{}", verb_type)
                            .as_bytes()
                            .to_vec(),
                        display,
                    }
        ...
        pub fn create_exercise_interaction(
            ...
            (
                ...
                verb,
                object,
                InteractionCategory::Exercise,
                Vec::new(),
            )
        }

        /// Get interactions by category
        pub fn get_interactions_by_category(
            category: InteractionCategory,
        ) -> Vec<LearningInteraction<T::AccountId>> {
            let mut interactions = Vec::new();
            let next_id = Self::next_interaction_id();
        ...
        pub fn get_course_hierarchy(
            course_id: CourseId,
        ) -> Option<(
            Course<T::AccountId>,
            Vec<(Class<T::AccountId>, Vec<TutorialSession<T::AccountId>>)>,
        )> {
        ...
        pub fn get_student_learning_path(
            student: &T::AccountId,
        ) -> Vec<(u64, String, Vec<InteractionId>)> {
            ...
            for id in 0..next_class_id {
                if let Some(class) = Self::classes(id) {
                    let user_interactions: Vec<InteractionId> = class
                        .interactions
                        .iter()
