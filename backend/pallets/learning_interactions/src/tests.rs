...
fn create_course_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(Event::CourseCreated { 
			course_id: 0, 
			instructor: 1 
		}.into());
	});
...
fn create_class_works() {
	new_test_ext().execute_with(|| {
		...
		// Assert that the correct event was deposited
		System::assert_last_event(Event::ClassCreated { 
			class_id: 0, 
			instructor: 1,
			course_id: None
		}.into());
...
fn create_tutorial_session_works() {
	new_test_ext().execute_with(|| {
		...
		// Assert that the correct event was deposited
		System::assert_last_event(Event::TutorialSessionCreated { 
			tutorial_id: 0, 
			tutor: 1,
			student: 2
		}.into());
...
fn end_tutorial_session_works() {
	new_test_ext().execute_with(|| {
		...
		// Assert that the correct event was deposited
		System::assert_last_event(Event::TutorialSessionEnded { 
			tutorial_id: 0
		}.into());
	});
}
...
fn session_already_ended_error() {
	new_test_ext().execute_with(|| {
		...
		// Try to end it again
		assert_noop!(
			LearningInteractions::end_tutorial_session(
				RuntimeOrigin::signed(1),
				0
			),
...
fn create_interaction_works() {
	new_test_ext().execute_with(|| {
		...
		// Assert that the correct event was deposited
		System::assert_last_event(Event::InteractionCreated { 
			interaction_id: 0, 
			actor: 1,
...
fn update_course_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(course.description, b"Advanced introduction to mathematics".to_vec());
...
fn create_question_interaction_helper_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(evidence[0].content, b"What is the derivative of x^2?".to_vec());
...
fn get_interactions_by_category_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LearningInteractions::create_interaction(
			RuntimeOrigin::signed(1), actor1, verb1, object1, None, None, category1, evidence1, None, None
		));
		
		assert_ok!(LearningInteractions::create_interaction(
			RuntimeOrigin::signed(2), actor2, verb2, object2, None, None, category2, evidence2, None, None
		));
		
		// Get interactions by category
		let questions = LearningInteractions::get_interactions_by_category(crate::InteractionCategory::Question);
		let answers = LearningInteractions::get_interactions_by_category(crate::InteractionCategory::Answer);
